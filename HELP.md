...
| ------------ | ---------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ |
| **`bdos`**   | `BDOS_Call`, `BDOS_PrintChar`, `BDOS_PrintString`, `BDOS_ReadChar`, `BDOS_Exit`, `BDOS_FileOpen`, `BDOS_FileCreate`, `BDOS_FileClose`, `BDOS_FileRead`, `BDOS_FileWrite`, `BDOS_FileSeek` | Chamadas diretas ao kernel MSX-DOS (BDOS 0x0005), incluindo I/O de arquivo baseado em handle (MSX-DOS 2, funções 43h-4Ah). |
...
| **`vdp`**    | `VDP_WriteReg`, `VDP_SetWriteAddr`, `VDP_SetReadAddr`, `VDP_FillVRAM`, `VDP_WriteVRAM`, `VDP_ReadVRAM`, `VDP_SetColor`, `VDP_SpriteDefine`, `VDP_SpriteSet`, `VDP_SpriteHide`, `VDP_SpriteHideAll`, `VDP_SpriteSetSize` | Manipulação das portas I/O do processador de vídeo TMS9918 / V9938 / V9958, acesso direto à VRAM e sprites (Sprite Mode 1, 8x8/16x16) em SCREEN 2. |
| **`psg`**    | `PSG_Write`, `PSG_Read`, `PSG_MuteAll`, `PSG_PlayTone`, `PSG_PlayNoteIndexed`, `PSG_PlaySequence`                     | Controle dos registradores de som do AY-3-8910 / YM2149, reprodução de notas por nome/oitava via tabela de períodos, e sequências simples de melodia. |
